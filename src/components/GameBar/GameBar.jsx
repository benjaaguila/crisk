...
export default function GameBar() {
    ...
    const handleThrowDice = () => {
        ...
        const config = {
            'method': 'get',
            'url': `${VITE_BACKEND_URL}/games/${gameId}/throwDice`,
            'headers': {
        ...
        axios(config)
        .then((response) => {
            setDiceValue(response.data.diceNumber);
        }
        )
        .catch((error) => {
            console.log(error);
    ...
    if (Object.keys(playerData).length !== 0 && Object.keys(gameData).length !== 0 && Object.keys(gameStatus).length !== 0) {
